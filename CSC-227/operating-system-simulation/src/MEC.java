...
public class MEC {
    ...
    private static void fillFileWithProcesses() {
        try {
            ...
            while (programmeSize < hardDisk) {
                ...
                if (programmeSize >= hardDisk) break;
    ...
    private static void fillProbability() {
        ...
        for (int i = 0; i < probability.length; i++) {
            if (i < 5) probability[i] = Probability.TerminatesNormally; else if (i == 5) probability[i] =
                Probability.TerminatesAbnormally; else if (i >= 6 && i <= 15) probability[i] =
                Probability.Interrupt; else if (i >= 16 && i <= 35) probability[i] = Probability.IORequest; else if (
                i >= 36 && i <= 55
            ) probability[i] = Probability.BusyIODevice; else probability[i] = Probability.DoNothing;
        }
    }

    private static void fillRAM() {
        while (RAM > 0 && JobQueue.peek() != null) {
            if ((RAM - JobQueue.peek().size) <= 0) return;
    ...
    private static void sorting() {
        ...
        while ((process = ReadyQueue.poll()) != null) arrayList.add(process); // fill the array of processes from the queue

        Collections.sort(arrayList); // then sorting the array

        for (PCB element : arrayList) ReadyQueue.add(element); // refill the readyQueue to start execution
    }
    ...
    private static void startMachineExecutionCycle() {
        while (ReadyQueue.peek() != null) {
            ...
            for (int i = currentProcess.counterExecutionTime; i <= currentProcess.expectedExecutionTime; i++) {
                ...
                if (
                    x == Probability.TerminatesNormally ||
                    x == Probability.TerminatesAbnormally ||
                    x == Probability.Interrupt
                ) break;
            }
    ...
    private static void IORequestCase() {
        if (currentProcess.counterIOTime >= currentProcess.expectedIOTime) return;
        ...
        while (currentProcess.counterIOTime < currentProcess.expectedIOTime) {
            ...
            if (x == Probability.BusyIODevice) busyIODeviceCase();
        }
    ...
    private static void isFinished() {
        if (
            currentProcess.counterExecutionTime >= currentProcess.expectedExecutionTime &&
            currentProcess.state == State.Running
        ) {
            currentProcess.state = State.TerminatedNormally;
            processes.get(currentProcess.id).state = State.TerminatedNormally;
    ...
    private static void outputFromTheSimulation() {
        ...
        for (PCB element : processes) {
            ...
            if (element.state == State.TerminatedNormally) processesNormally++; else if (
                element.state == State.TerminatedAbnormally
            ) processesAbnormally++;

            if (element.counterExecutionTime > element.counterIOTime) processesCPUBound++;
        }
        ...
        System.out.println(
            ...
            ((1.0 * processesNormally) / (1.0 * processes.size()))
        );
        System.out.println(
            "4. The average number of jobs that have completed their execution abnormally : " +
            ((1.0 * processesAbnormally) / (1.0 * processes.size()))
        );

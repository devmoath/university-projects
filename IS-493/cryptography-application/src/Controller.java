...
public class Controller {
    ...
    private void createLink(FXMLLoader loader) {
        ...
        Background defaultBackground = new Background(
            new BackgroundFill(Color.CADETBLUE, new CornerRadii(5), Insets.EMPTY)
        );
        Background hoveredBackground = new Background(
            new BackgroundFill(Color.DARKCYAN, new CornerRadii(5), Insets.EMPTY)
        );

        my_website.setTextFill(Color.WHITE);
        my_website.setBackground(defaultBackground);

        my_website.addEventHandler(
        ...
        my_website.setOnAction(
            new EventHandler<ActionEvent>() {
                @Override
                public void handle(ActionEvent event) {
                    String url = "https://moath.dev/";
                    try {
    ...
    private void enterButton() {
        if (
            ...
            textField.getText().equals("e.g. Hello") |
            keywordField.getText().isEmpty() |
            keywordField.getText().equals("e.g. Act")
        ) {

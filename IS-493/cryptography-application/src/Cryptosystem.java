...
public class Cryptosystem {
    ...
    private Key[] CreateKeyObjects(String keyword) {
        ...
        for (int i = 0; i < keyword.length(); i++) {
            for (int j = 0; j < alphabet.length(); j++) if (
                String.valueOf(keyword.charAt(i)).equalsIgnoreCase(String.valueOf(alphabet.charAt(j)))
            ) keys[i] = new Key(j, i);
        }
    ...
    public String encryption(String plaintext, String keyword) throws Exception {
        ...
        while (counter < keys.length) {
            ...
            for (int i = 0; i < array.length; i++) ciphertext += array[i][key];
    ...
    public String decryption(String ciphrtext, String keyword) throws Exception {
        ...
        for (int i = 0; i < cipher.length; i++) {
            key = keys[i].position;
            for (int j = 0; j < cipher[i].length(); j++) array[j][key] = cipher[i].charAt(j);
        }

        String plaintext = "";

        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < array[i].length; j++) plaintext += array[i][j];
        }
